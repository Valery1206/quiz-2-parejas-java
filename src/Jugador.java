...
public class Jugador {
    ...
    public String getGrupos() {
        String mensaje = "No se tuvo ningun grupo";
        ...
        if (hayGrupos) {
            mensaje = "Se pudo encontra estos grupos:\n";
            int p = 0;
            for (int c : contadores) {
                if (c >= 2) {
...
listaCartas.sort(Comparator.comparing(Carta::getPinta).thenComparing(Carta::getNombre));
    ...
    for (Carta carta : listaCartas) {
        ...
        if (secuencia.size() >= 3) {
            mensajeEscaleras += "Vea tiene Escalera de " + pintaActual + ": " + secuencia + "\n";
        }
    ...
    mensaje += "\nEscaleras encontradas andamos con suerte:\n" + mensajeEscaleras;
    }else {
    mensaje += "\nNo tienes nada de Escaleras mal ahi sigue intentado :c ";
    }
    return mensaje;
    }
